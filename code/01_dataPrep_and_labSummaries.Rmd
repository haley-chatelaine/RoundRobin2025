...
dup.dist <- do.call(rbind, lapply(dup, function(x) mean(x$value, na.rm = T)))

nondup <- lapply(euc.data, function(x) x[which(is.na(x$dup) &
                                              x$value != 0 &
                                              !is.na(x$value)),])
nondup.dist <- do.call(rbind, lapply(nondup, function(x) mean(x$value, na.rm = T)))

rel.dist <- dup.dist / nondup.dist
```
