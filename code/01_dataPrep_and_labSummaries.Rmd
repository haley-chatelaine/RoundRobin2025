...
c13.results <- read.results(path = path, file.text = "_Norm", sheet.text = "C13-MSTUS")

# Raw mstus-filtered

raw.mstus <- read.results(path = path, file.text = "_Norm", sheet.text = "C12-MSTUS")
...
# Calculate CVs of C12 and C13 raw values

```{r}
c13.cvs <- lapply(c13.results, calc.cv)
lapply(c13.cvs, mean)
lapply(c13.cvs, sd)

c12.cvs <- lapply(raw.mstus, calc.cv)
lapply(c12.cvs, mean)
lapply(c12.cvs, sd)
```
...
```{r}
mstus.impute <- lapply(raw.impute, function (x) chris.mstus(x, low.boun = 0.2, high.bound = 0.95))
```
...
cor.stats <- lapply(norm.cor, function(x) paste(round(mean(x$correlation), 2),
                                   "+/-",
                                   round(sd(x$correlation), 2)))
do.call(rbind, cor.stats)
```
...
# Euclidean distance of sample pairs

```{r}
# raw distances
euc.data <- lapply(norm.impute, euclid.func)

# scaled to number of features
for(lab in 1:length(euc.data)){
  
  euc.data[[lab]]$feat.scale.value <- euc.data[[lab]]$value / sqrt(ncol(norm.impute[[lab]]))  
  
}

# scaled to max Euclidean distance
for(lab in 1:length(euc.data)){
  
  euc.data[[lab]]$max.scale.value <- euc.data[[lab]]$value / max(euc.data[[lab]]$value)  
...
```{r}
# heatmaps
#euc.plots <- mapply(function(x, y) euclid.func(x, y), norm.impute, names(norm.impute))
#ggarrange(plotlist = euc.plots, nrow = 2, ncol = 4)

# boxplots
euclid.box(euc.data, data.type = "raw", meta.data)
ggsave("/Users/chatelaineha/Documents/Beecher_Collaboration/Writing/data_code/outputs/boxplots_euclidean_distances.png", width = 16, height = 12, dpi = 600)

euclid.box(euc.data, data.type = "feat.scaled", meta.data)

euclid.box(euc.data, data.type = "max.scaled", meta.data)

# Plot scaled distances in one plot

## mark duplicates
plot.data <- lapply(euc.data, mark.dup)

## remove same samples
plot.data<- lapply(plot.data, function (x) x[-which(x$dup == "X" & x$value == 0),])

## make one dataframe
plot.data <- do.call(rbind, lapply(names(plot.data), function(nm) {
  df <- plot.data[[nm]]
  data.frame(max.scale.value = df$max.scale.value,
             dup = df$dup,
             lab = nm)
}))
...
ggplot(plot.data, aes(x = lab, y = max.scale.value, fill = dup_status)) +
...
## Summary stats of Euclidean distances

```{r}
euc.data <- lapply(euc.data, mark.dup)

# mean true duplicate distances
dup <- lapply(euc.data, function(x) x[which(x$dup == "X" &
                                              x$value != 0 &
                                              !is.na(x$value)),])

dup.dist.mean <- do.call(rbind, lapply(dup, function(x) mean(x$value, na.rm = T)))

nondup <- lapply(euc.data, function(x) x[which(is.na(x$dup) &
                                              x$value != 0 &
                                              !is.na(x$value)),])

nondup.dist.mean <- do.call(rbind, lapply(nondup, function(x) mean(x$value, na.rm = T)))

# T-test of true vs. non-duplicates

dup.dist <- do.call(rbind, lapply(dup, function(x) x$value))

nondup.dist <- do.call(rbind, lapply(nondup, function(x) x$value))

ttest.dist <- data.frame(matrix(nrow = nrow(dup.dist)))

for(row in 1:nrow(dup.dist)){
  
  test <- t.test(log2(dup.dist[row,]), log2(nondup.dist[row,]))
  
  pval <- test$p.value

  ttest.dist[row,] <- format(pval, scientific = T)
}
