...
mean.8 <- mean.8[which(colnames(data.8) %in% high.8)]

boxplot(all.means)
boxplot(mean.8)

# boxplots based on features with high coefficients

par(mfrow = c(2, 4))
for(i in 1:length(norm.impute)){
  
  res <- nmf(log2(norm.impute[[i]]), rank = 3)
  h.i <- res@fit@H
  h.i.means <- colMeans(h.i)
  max.h.i <- log2(norm.impute[[i]])[,which(h.i.means > quantile(h.i.means, 0.75))]
...
par(mfrow = c(2,4))
lapply(norm.impute, boxplot)

# abundance relationship with coefficients
...
for(i in 1:length(norm.impute)){
  ...
  h.i <- res@fit@H
  h.i.means <- colMeans(h.i)
  data.i.means <- colMeans(log2(norm.impute[[i]]))
  
  plot(data.i.means ~ h.i.means)
...
for(i in 1:length(norm.impute)){
  ...
  res <- nmf(norm.impute[[i]], rank = 3)
  h.i <- res@fit@H
  h.i.means <- colMeans(h.i)
  data.i.means <- colMeans(norm.impute[[i]])
